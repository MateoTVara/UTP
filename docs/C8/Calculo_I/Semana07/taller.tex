...
\begin{tikzpicture}
  ...
  % Recta 2x + 8 + y = 0
  \addplot[red, very thick] coordinates {(-5,2)(-3,-2)};
  \addplot[red, mark=o, only marks, mark size=2pt] coordinates {(-5,2) (-3,-2)};

  % Punto cerrado (-3,1)
  \addplot[red, mark=*, only marks, mark size=2pt] coordinates {(-3,1)};

  % Parabola -x^2 - 2x + 2
  \addplot[red, very thick, domain=-3:0, samples=100] {-x^2 - 2*x + 2};
  \addplot[red, mark=o, only marks, mark size=2pt] coordinates {(-3,-1) (0,2)};

  % Parabola -x^2 + 2x + 1
  \addplot[red, very thick, domain=0:3, samples=100] {-x^2 + 2*x + 1};
  \addplot[red, mark=o, only marks, mark size=2pt] coordinates {(0,1) (3,-2)};

  % Punto cerrado (0,-2)
  \addplot[red, mark=*, only marks, mark size=2pt] coordinates {(0, -2)};

  \end{axis}
\end{tikzpicture}
\end{center}

\noindent Determine el valor de:
...
\begin{array}{c|c}
  \begin{array}{c}
    \dfrac
    {\lim_{x \to -3^+} f(x) - \lim_{x \to -2^-} f(x) - \lim_{x \to 0^+} f(x)}
    {2 f(0) - 1}
    \\\\
    \dfrac{-1 - (2) - (1)}{2(-2) - 1} = \dfrac{-4}{-5} = \mathbf{\dfrac{4}{5}}
  \end{array}
  &
  \begin{array}{c}
    \dfrac
    {\lim_{x \to -5^+}f(x) - \lim_{x \to -3^-} f(x) - \lim_{x \to 2} f(x)}
    {f(-3) - 2}
    \\\\
    \dfrac{2 - (-2) - (1)}{1 - 2} = \dfrac{3}{-1} = \mathbf{-3}
  \end{array}
\end{array}
$}
\]
...
\begin{itemize}
    \item \textbf{Código fuente:} \href{https://github.com/MateoTVara/UTP/blob/main/docs/C8/Calculo_I/Semana07/taller.pdf}{Repositorio GitHub - Cálculo I}
